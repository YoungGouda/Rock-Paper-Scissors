...
struct Label
{
	Label(const SDL_Rect pos, SDL_Texture * tex)
...
public:
	...
	void set_label_text(std::string text, SDL_Rect position, const std::string font)
	{
		...
		label_creator_.push_back(new Label(position, label_texture));
	}
	...
	void set_label_text(Link * link)
	{
		...
		label_creator_.push_back(new Label(link->get_link_dimensions(), label_texture));
	}
...
private:
	SDL_Rect position_;
	std::vector<Label *> label_creator_;
