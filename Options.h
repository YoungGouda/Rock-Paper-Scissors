...
class Options
{
	//this will be moved to a txt file
	std::vector<const char *> link_assets_{ "Free Play", "Volume", "Difficulty: ", "I do not like the current state of economics", "Attack" };

	std::vector<Link *> links_ {new CursorLeftLink(new FreePlayCommand()), new CursorLeftLink(new NothingCommand()), new CursorLeftLink(new NothingCommand()), new CursorLeftLink(new NothingCommand()) };
...
public:
	Options(Choices choices)
	{
		...
		links_[choices.options[0][0].link]->set_link_information(choices.link_parameters, choices.options[0][0].position, link_assets_[choices.options[0][0].link], starting_rect);
		...
		for (auto x = 0; x < choices.options.size(); x++)
		{
			...
			for (auto y = 0; y < choices.options[x].size(); y++)
			{
				...
				current_link->set_link_information(choices.link_parameters, choices.options[x][y].position, link_assets_[choices.options[x][y].link], starting_rect);
				...
				// setting up the option_rect size 
